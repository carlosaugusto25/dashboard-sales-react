//COMPONENTS
import { CardComponent, Header, AvatarsList, CustomTable, CustomChart, StyledH2, StyledH3, StyledSpan } from "@/components";
...
//TYPES
import { HighlightsDataProps, StarsDataProps, NewsDataProps, CustomChartProps } from "@/types";
            ...
            <Container maxWidth='lg'>
                <Grid container spacing={4}>
                                ...
                                <CardComponent id='total-sales' className={highlightsLoading ? 'skeleton-loading skeleton-loading-mh-1' : ''}>
                                    {
                                        !highlightsLoading && highlightsData && (
                                            <>
                                                ...
                                                <StyledH3 size={40} lineheight={40} className="mb-1">{currencyConverter(highlightsData[0]?.value)}</StyledH3>
                                ...
                                <CardComponent id='month-goals' className={highlightsData ? highlightsData[1].subtitle : 'skeleton-loading skeleton-loading-mh-1'}>
                                    {
                                        !highlightsLoading && highlightsData && (
                                            <>
                                                ...
                                                <StyledH3 size={40} lineheight={40} className="mb-1" color="white">{currencyConverter(highlightsData[1]?.value)}</StyledH3>
                                ...
                                <CardComponent id='total-leads' className={highlightsLoading ? 'skeleton-loading skeleton-loading-mh-1' : ''}>
                                    {
                                        !highlightsLoading && highlightsData && (
                                            <>
                                                ...
                                                <StyledH3 size={40} lineheight={40} className="mb-1">{highlightsData[2]?.value}</StyledH3>
                    ...
                    <Grid item xs={12} sm={5}>
                        {
                            !salesStarsError && (
                                <CardComponent id='sales-stars' className={salesStarsLoading ? 'skeleton-loading skeleton-loading-mh-2' : ''} >
                                    {
                                        !salesStarsLoading && salesStarsData && (
                                            <>
                                                <StyledH2 className="mb-1">Maiores vendeores do mês</StyledH2>
                                            ...
                                            <>
                                                ...
                                                <CustomTable
                                                    ...
                                                    rows={newsData.map(news => [
                                                        <a className="ellipsis ellipsis-sm" href={news.link} target="_blank" rel="noreferrer">{news.title}</a>,
                                                        <a href={news.link} target="_blank" rel="noreferrer">{news.date}</a>
                                                    ])}
                                                />
                                            </>
                                        )
